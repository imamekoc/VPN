...
# // Fetch IP and Permission Data (Optimized)
if [[ -z "$MYIP" || -z "$ISPVPS" || -z "$IPVPS" || -z "$IP" ]]; then
    IP_DATA=$(curl -s ipinfo.io/json)
    # Parse JSON using grep and sed to avoid jq dependency
    FETCHED_IP=$(echo "$IP_DATA" | grep '"ip":' | sed -E 's/.*"ip": "([^"]+)".*/\1/')
    FETCHED_ORG=$(echo "$IP_DATA" | grep '"org":' | sed -E 's/.*"org": "([^"]+)".*/\1/')

    export MYIP="$FETCHED_IP"
    export IP="$FETCHED_IP"
    export IPVPS="$FETCHED_IP"
    export ISPVPS="$FETCHED_ORG"
fi

if [[ -z "$Name" || -z "$Exp" ]]; then
    IZIN_DATA=$(curl -sS https://raw.githubusercontent.com/imamekoc/VPN/main/izin)
    export Name=$(echo "$IZIN_DATA" | grep "$MYIP" | awk '{print $2}')
    export Exp=$(echo "$IZIN_DATA" | grep "$MYIP" | awk '{print $3}')
fi
