...
# CF_ID and CF_KEY should be set in environment or prompted
if [ -z "${CF_ID:-}" ]; then
  echo "Cloudflare Email not found in environment."
  read -p "Enter Cloudflare Email: " CF_ID
fi

if [ -z "${CF_KEY:-}" ]; then
  echo "Cloudflare Global API Key not found in environment."
